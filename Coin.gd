...
var c = 0
func _ready():
	$Tween.interpolate_property(self, "scale", Vector2.ZERO, Vector2(1,1), 0.25, Tween.TRANS_EXPO, Tween.EASE_IN)
	$Tween.start()
	...
	$Tween.start()
	$Tween.interpolate_property(self, "modulate:a", 1.0, 0.0, 2.0, Tween.TRANS_EXPO, Tween.EASE_IN, 0.25)
	$Tween.start()
